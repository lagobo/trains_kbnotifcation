...
KB390L::KB390L(int aVendor, int aProduct, QObject *parent)
    : QObject(parent)
    , device(new QHIDDevice(aVendor ? aVendor : VENDOR, aProduct ? aProduct : PRODUCT, GENERIC_USAGE_PAGE, GENERIC_USAGE, this))
    , eventDevice(new QHIDDevice(aVendor ? aVendor : VENDOR, aProduct ? aProduct : PRODUCT, EVENT_USAGE_PAGE, EVENT_USAGE, this))
    , monitor(new QHIDMonitor(aVendor ? aVendor : VENDOR, aProduct ? aProduct : PRODUCT, this))
    , timerId(0)
{
    ...
    {
        throw "Device not found or accessible";
    }
}
...
void KB390L::setColot(int R, int G, int B)
{
    setFlag(CmdColor, R & 0xff, LightRedOffset);
    setFlag(CmdColor, G & 0xff, LightGreenOffset);
    setFlag(CmdColor, B & 0xff, LightBlueOffset);
}
